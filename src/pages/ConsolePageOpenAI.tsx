...
export function ConsolePageOpenAI() {
  const {
    setResponseBuffer,
    connectStatus,
    setConnectStatus,
  ...
  */
  ...
  const sendAssistantMessage = async (text: string) => {
    ...
    try {
      ...
      console.log('messagesAssistant', messagesAssistant);
      ...
      const stream = await getOpenAIClient().chat.completions.create({
        ...
        messages: [
          { role: 'system', content: llmInstructionsRef.current },
          ...messagesAssistant.map(item => ({role: item.role, content: item.text})),
          {
            role: 'user',
            content: text,
          },
      ...
      for await (const event of stream) {
        // console.log('event', event, event.usage);
